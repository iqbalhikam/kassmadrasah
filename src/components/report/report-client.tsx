...
export function ReportClient({ initialData, initialError = "" }: ReportClientProps) {
  ...
  const defaultDates = useMemo(() => {
    if (!initialData?.transaksi.length) return { from: "", to: "" };
    const txDates = initialData.transaksi.map((t: Transaksi) => t.tanggal).filter(Boolean).sort();
    return {
      from: txDates[0] || "",
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <ModernDatePicker
                    ...
                    size="sm"
                    value={dateFrom}
                    onChange={(val) => { setDateFrom(val); setFilterApplied(false); }}
                ...
                <div>
                  ...
                  <ModernDatePicker
                    ...
                    size="sm"
                    value={dateTo}
                    onChange={(val) => { setDateTo(val); setFilterApplied(false); }}
